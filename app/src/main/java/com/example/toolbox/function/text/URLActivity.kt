        ...
        setContent {
            ToolBoxTheme {
                Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
                    URLScreen(modifier = Modifier.padding(bottom = innerPadding.calculateBottomPadding()))
                }
            }
        }
    }
...
@Composable
fun URLScreen(modifier: Modifier = Modifier) {
    ...
    fun encodeToURL() {
        ...
        try {
            if (inputText.isNotEmpty()) {
                val encoded = URLEncoder.encode(inputText, StandardCharsets.UTF_8.toString())
                outputText = encoded
            } else {
    ...
    fun decodeFromURL() {
        ...
        try {
            if (inputText.isNotEmpty()) {
                val decoded = URLDecoder.decode(inputText, StandardCharsets.UTF_8.toString())
                outputText = decoded
            } else {
                errorMessage = "请输入要解码的URL编码文本"
            }
        } catch (e: Exception) {
            errorMessage = "解码失败: 请检查输入是否为有效的URL编码"
        }
    }
